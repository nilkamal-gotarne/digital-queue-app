...
export default function JoinQueue() {
    ...
    if (result.data) {
      ...
      try {
        ...
        const queuesSnapshot = await getDocs(
          query(collection(db, "queues"), where("adminId", "==", result.data))
        );
        ...
        if (!queuesSnapshot.empty) {
          ...
          for (const queueDoc of queuesSnapshot.docs) {
            ...
            if (queueLength < shortestQueueLength) {
              ...
              shortestQueueMembers = membersSnapshot.docs.map((doc) =>
                doc.data()
              );
            }
          }
  ...
  const handleJoinQueue = async () => {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        // Get the last member in the queue to determine the next position
        const queueMembersQuery = query(
          queueMembersRef,
          where("queueId", "==", shortestQueue.id),
          where("lotId", "==", lotData.id),
        ...
        // Check if the new position is already taken (atomic check)
        const positionQuery = query(
          queueMembersRef,
          where("queueId", "==", shortestQueue.id),
          where("lotId", "==", lotData.id),
        ...
        const joinTime = serverTimestamp();
        const endTime = moment(lastPersonEndTime)
          .add(sessionData.avgWaitingTime, "minutes")
          .toDate();
        const waitingTime = moment(endTime).diff(moment(), "minutes");
        ...
        const newDocRef = doc(queueMembersRef);
        transaction.set(newDocRef, newQueueMember);
      });
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View
        style={scanned ? styles.infoContainerScanned : styles.infoContainer}
      >
        ...
        {isLoading ? (
          ...
          scanned && (
            <>
              ...
              {queueName && (
                <Text style={styles.infoText}>Queue: {queueName}</Text>
              )}
              {ownerName && (
